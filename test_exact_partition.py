...
def test_exact_partition():
    ...
    for L, j, beta in test_cases:
        our_result = our_logZ(n=L, j=j, beta=torch.tensor(beta, dtype=torch.float64)).item()
        ref_result = ref_logZ(n=L, j=j, beta=torch.tensor(beta, dtype=torch.float64)).item()
        ...
        print(f"{L:>3} {beta:>6.3f} {our_result:>14.6f} {ref_result:>14.6f} "
