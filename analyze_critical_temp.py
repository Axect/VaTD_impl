...
def analyze_around_critical(model, energy_fn, L, device='cpu', num_temps=20, batch_size=500):
    """
...
def plot_critical_analysis(df, output_dir='plots'):
    ...
    ax.axvline(CRITICAL_TEMPERATURE, color='r', linestyle='--', alpha=0.5, label=f'Tc = {CRITICAL_TEMPERATURE:.3f}')
...
def main():
    ...
    print(f"\nBest accuracy:")
    print(f"  T = {df.loc[idx_min_error, 'T']:.4f} (T/Tc = {df.loc[idx_min_error, 'T/Tc']:.3f})")
    print(f"  |Error| = {df.loc[idx_min_error, 'abs_error']:.6f}")

    print(f"\nWorst accuracy:")
    print(f"  T = {df.loc[idx_max_error, 'T']:.4f} (T/Tc = {df.loc[idx_max_error, 'T/Tc']:.3f})")
